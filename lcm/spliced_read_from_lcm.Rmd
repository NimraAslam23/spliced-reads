...
```{r disease group column, include=FALSE}
...
```{r density plot p-value, include = FALSE}
...
```{r density plot - significant p value, include = FALSE}
...
```{r density plot - significant spliced reads in ALS and controls, echo = FALSE}
...
Both the ALS and control cohorts have expression of mostly annotated junctions. ALS LCM neurones also exhibit expression of completely novel cryptic exons (with neither end overlapping a known exon) and novel acceptor junctions, with one end overlapping that of a known exon. Almost all junction events are seen in control LCM neurones. 
...
The annotated events found more highly expressed in control motor neurons were unique to a single TDP-43 knockdown, while the junction events in ALS motor neurons were more commonly found in studies (Table 2). This means there is more evidence supporting the expression of the junction events found in ALS LCM neurones. 
