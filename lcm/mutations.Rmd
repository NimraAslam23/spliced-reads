...
```{r pull back all mutation data from TCGA, include = FALSE}
# code to download data is in R script file 
all_mutation_data_orig <- read.table("all_mutation_data.txt", sep="", header = T)
all_mutation_data <- all_mutation_data_orig
```


```{r left join clinical df to mutation df, include = FALSE}
