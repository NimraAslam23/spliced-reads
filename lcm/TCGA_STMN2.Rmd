...
```{r calculating mutation burden (weighting total mutations against total cases) for general vs cryptic, echo = FALSE, warning = FALSE}
...
cBio_clinical |>
  ...
  stat_compare_means(comparisons = list(c("gbm", "lgg")), label = "p.format")
...
```{r survival KM curve for cases with cryptic, echo = FALSE}
survfit(Surv(months_of_disease_specific_survival, disease_specific_survival_status) ~ 1, 
  ...
  labs(
    ...
    title = "No Cryptic Events"
  ) +
  add_confidence_interval() 
```
**Figure 11**:

```{r survival KM curve for cases with no cryptic, echo = FALSE}
survfit(Surv(months_of_disease_specific_survival, disease_specific_survival_status) ~ 1, 
  ...
  labs(
    x = "Months",
    y = "Overall Survival Probability",
    title = "Cryptic Events"
  ) +
  add_confidence_interval() 
```
**Figure 12**:

```{r survival KM curves - both cryptic and non, echo = FALSE}
...
```{r density plot - months survival in cryptic vs non-cryptic, echo = FALSE}
...
```{r PCPG survival analysis, echo = FALSE}
...
```{r GBM survival analysis, echo = FALSE}
survfit(Surv(months_of_disease_specific_survival, stmn2_cryptic_detected) ~ stmn2_cryptic_detected, 
